...
function defineDarkTheme(monaco: Monaco): void {
  monaco.editor.defineTheme('scipen-dark', {
    ...
    rules: [
      ...
      { token: 'keyword.structure', foreground: '93c5fd' }, // blue-300 (cool) replaces warm yellow
      ...
      { token: 'keyword.format', foreground: '5eead4' }, // teal (cool) replaces warm orange/brown
      { token: 'keyword.math', foreground: '10b981' }, // --color-success
      { token: 'string', foreground: '5eead4' }, // teal (cool) replaces warm orange/brown
      { token: 'string.math', foreground: '22d3ee' }, // cyan (cool) replaces amber
      { token: 'number', foreground: '10b981' }, // --color-success
      { token: 'delimiter.curly', foreground: '94a3b8' }, // slate (cool) replaces amber
      ...
      { token: 'markup.inlinecode', foreground: '22d3ee' }, // cyan (cool) replaces amber
    ...
    colors: {
      // Note: Monaco theme colors only accept #RRGGBB[AA] hex; rgba() silently falls back to default — use hex everywhere.
      'editor.background': '#18181b',
      'editor.foreground': '#fafafa',
      'editorCursor.foreground': '#22d3ee',
      'editor.lineHighlightBackground': '#27272a',
      // Selection: brand accent (soft cyan), replaces the previously broken rgba (bright cyan).
      ...
      // Diagnostics: soft danger/warning, replaces Monaco's default harsh red.
      'editorError.foreground': '#d88484',
      'editorWarning.foreground': '#d1a168',
      // Overview ruler (right-side "red bar" attenuated via alpha).
      ...
      // Minimap (right-side thumbnail: selection / error / warning / find match).
...
function defineLightTheme(monaco: Monaco): void {
  monaco.editor.defineTheme('scipen-solarized-light', {
    ...
    rules: [
      ...
      { token: 'keyword.control', foreground: 'c026d3' }, // fuchsia (cool) replaces academic-red
      { token: 'keyword.structure', foreground: '2563eb' }, // blue replaces academic-gold
      { token: 'keyword.section', foreground: '1e4e8c' }, // academic-blue (cool, kept)
      { token: 'keyword.reference', foreground: '0d7377' }, // academic-teal (cool, kept)
      { token: 'keyword.format', foreground: '0d9488' }, // teal replaces academic-gold
      { token: 'keyword.math', foreground: '4f46e5' }, // --color-info
      { token: 'string', foreground: '0d7377' }, // academic-teal (cool, kept)
      { token: 'string.math', foreground: '0d9488' }, // teal replaces academic-gold
      { token: 'number', foreground: '0891b2' }, // cyan replaces academic-red
      { token: 'delimiter.curly', foreground: '64748b' }, // slate replaces academic-gold
      ...
      { token: 'markup.inlinecode', foreground: '0d9488' }, // teal (cool) replaces academic-gold
    ...
    colors: {
      ...
      // Selection: quiet-blue accent (aligned with brand accent, drops legacy academic-navy).
